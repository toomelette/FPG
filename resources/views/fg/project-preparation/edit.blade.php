...
@section('content2')
    ...
    <form id="edit-project-preparation-form">
        <x-adminkit.html.card header-class="pt-3 pb-1" body-class="pt-2">
            ...
            <div class="row mb-2">
                <div class="col-md-3">
                    <div class="row mb-2">
                        <x-forms.input label="Control No." name="control_no" cols="6" :value="$pp ?? null"/>
                        <x-forms.input label="Date" name="date" cols="6" type="date" :value="$pp ?? null"/>
                    </div>
                    <div class="row mb-2">
                        <x-forms.select label="Project" name="invoice_uuid" cols="12" :options="[]" id="select2-project" :value="$pp ?? null" select2-preselected="{{$pp?->invoice?->remarks.' - '.$pp?->invoice?->invoice_no}}"/>
                    </div>
                    <div class="row mb-2">
                        <x-forms.textarea label="Remarks" name="remarks" cols="12" :value="$pp ?? null"/>
                    </div>
                </div>
                <div class="col-md-9">
                    <table class="table table-striped table-sm table-bordered" id="details-table">
                        ...
                        <tr>
                            ...
                            <td class="align-top text-strong text-end">
                                <span id="grandTotal">{{number_format($pp->details_sum_amount,2)}}</span>
                            </td>
                            <td></td>
                        </tr>
                        </tfoot>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#edit-project-preparation-form").submit(function (e) {
            e.preventDefault();
            let form = $(this);
            loading_btn(form);
            $.ajax({
                url : '{{route("project-preparation.update",$pp->uuid)}}',
                data : form.serialize(),
                type: 'PATCH',
                headers: {
                    {!! __html::token_header() !!}
