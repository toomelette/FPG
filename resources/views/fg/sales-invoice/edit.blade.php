    ...
    <form id="edit-sales-invoice-form">
        <x-adminkit.html.card header-class="pt-3 pb-1" body-class="pt-2">
            ...
            <div class="row mt-2">
                ...
                <x-forms.textarea label="Remarks" name="remarks" cols="4" :value="$salesInvoice ?? null"/>
            </div>
            <div class="row mt-2">
                <div class="col-md-10">
                        ...
                        <tbody>
                            @forelse($salesInvoice->details as $detail)
                                <tr id="details-{{$detail->id}}" data-id="{{$detail->id}}">
                                    <td class="align-top">
                                        <x-forms.select :select-only="true" :auto-class="true" class="select2-ajax-auto-populate" :s2-id="$detail->stock_uuid ?? $detail->description" :s2-text="$detail->description" :s2-url='route("dashboard.ajax.get","stocks")' label="A" name="details[{{$detail->id}}][description]" cols="12"/>
                                    </td>
                                    <td class="align-top">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#edit-sales-invoice-form").submit(function (e) {
            e.preventDefault();
            let form = $(this);
            loading_btn(form);
            $.ajax({
                url : '{{route("sales-invoice.update",$salesInvoice->uuid)}}',
                data : form.serialize(),
                type: 'PUT',
                headers: {
                    {!! __html::token_header() !!}
                ...
                success: function (res) {
                    ...
                    toast('success','Sales Invoice successfully saved.','Success');
                },
                error: function (res) {
                    errored(form,res);
                }
