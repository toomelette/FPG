        ...
        <table style="width: 100%; font-size: 14px; margin-top: 10px" class="tbl-padded">
            ...
            @forelse($months as $month => $books)
                @forelse($books as $book => $entries)
                    <tr>
                        <td>{{Carbon::parse($month)->format('M d, Y')}}</td>
                        <td>{{$book}}</td>
                        <td class="text-right">{{Helper::toNumber($debit = $entries->sum('debit'))}}</td>
                        <td class="text-right">{{Helper::toNumber($credit = $entries->sum('credit'))}}</td>
                        <td class="text-right">{{Helper::toNumber($runningBalance = $runningBalance + $debit - $credit)}}</td>
                    </tr>
                @empty
                @endforelse
            @empty
