...
@section('content')
    ...
    <section class="content-header">
        <h1>Cash Disbursements <small>Journal Entry Voucher</small>
            <span class="pull-right">JEV: {{$cashDisbursement->jev_no}}</span>
        </h1>
    </section>
@endsection
@section('content2')
            ...
            <div class="box box-solid">
                <div class="box-header with-border">
                    ...
                    <div class="btn-group pull-right">
                        <a type="button" class="btn btn-sm btn-default" href="{{route('dashboard.cash_disbursements.print',$cashDisbursement->slug)}}" target="_blank"><i class="fa fa-print"></i> Print JEV</a>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab_1">
                                <table id="jev_details_table" class="table table-condensed table-bordered table-striped">
                                    ...
                                    <tbody>
                                    @forelse($cashDisbursement->details as $jevDetail)
                                        <tr id="row_slug">
                                            <td>
                                    ...
                                    <tr>
                                        ...
                                        <th class="totals debit_total text-right">{{number_format($cashDisbursement->details->sum('jev_debit'),2)}}</th>
                                        <th class="totals credit_total text-right">{{number_format($cashDisbursement->details->sum('jev_credit'),2)}}</th>
                                    </tr>
                                    </tfoot>
                                </table>
                            </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $("#add_jev_form").submit(function (e) {
            ...
            loading_btn(form);
            $.ajax({
                url : '{{route("dashboard.cash_disbursements.update",$cashDisbursement->slug)}}',
                data : form.serialize(),
                type: 'PATCH',
                headers: {
                    {!! __html::token_header() !!}
                ...
                success: function (res) {
                    ...
                    markTabs(form);
                    toast('success','Cash Disbursement successfully updated.','Success!');
                },
                error: function (res) {
                    errored(form,res);
                    markTabs(form);
