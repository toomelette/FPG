    ...
    <ul style="padding-left: 15px; font-size: 12px; font-family: Consolas" class="{{$type == 'DV' ? 'text-blue': 'text-green'}}">
        @if($entries->count() > 0)
            @foreach($entries as $entry)
                <li data-toggle="popup" title="{{$entry->account_code}} - {{$entry->chartOfAccount->account_title ?? 'N/A'}}">
                    <span class="{{empty($entry->chartOfAccount) ? 'text-danger' : ''}}">                     {{$entry->account_code}} -</span>
