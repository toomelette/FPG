                    ...
                    <table class="table table-striped table-sm table-bordered" id="details-table">
                        ...
                        @forelse($purchaseOrder->details as $detail)
                            <tr id="details-{{$detail->id}}" data-id="{{$detail->id}}">
                                ...
                                <td class="align-top">
                                    <x-forms.select :select-only="true" :auto-class="true" class="select2-ajax-auto-populate allow-new-items" :s2-id="$detail->stock_uuid ?? $detail->description" :s2-text="$detail?->stock?->name ?? $detail->description" :s2-url='route("dashboard.ajax.get","stocks")' label="A" name="details[{{$detail->id}}][description]" cols="12"/>
                                </td>
                                <td class="align-top">
