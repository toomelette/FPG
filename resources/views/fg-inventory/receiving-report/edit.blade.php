...
@section('content2')
    <x-adminkit.html.page-title>
        <x-slot:title>Edit Receiving Report</x-slot:title>
        <x-slot:subtitle> {{$receivingReport->control_no}}</x-slot:subtitle>
    </x-adminkit.html.page-title>
    ...
    <form id="edit-receiving-report-form">
        <x-adminkit.html.card header-class="pt-3 pb-1" body-class="pt-2">
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="row">
                        <x-forms.input label="RR No." name="control_no" cols="6" :value="$receivingReport ?? null"/>
                        <x-forms.input label="RR Date." name="date" cols="6" type="date" :value="$receivingReport ?? null"/>
                    </div>
                    <div class="row mt-2">
                        <x-forms.input label="PO No." name="po_no" cols="6" :value="$receivingReport ?? null"/>
                        <x-forms.input label="Terms" name="terms" cols="6" :value="$receivingReport ?? null"/>
                    </div>
                    <div class="row mt-2">
                        <x-forms.input label="Supplier" name="supplier" cols="12" :value="$receivingReport ?? null"/>
                    </div>
                    <div class="row mt-2">
                        <x-forms.input label="Acct No." name="account_no" cols="6" :value="$receivingReport ?? null"/>
                        <x-forms.input label="Inv/DR No" name="inv_dr_no" cols="6" :value="$receivingReport ?? null"/>
                    </div>
                    <div class="row mt-2">
                        <x-forms.textarea label="Remarks" name="remarks" cols="12" :value="$receivingReport ?? null"/>
                    </div>

                    <div class="row mt-2">
                        <x-forms.input label="Total" name="total_amount_due" cols="4" class="autonum-total-amount-due text-end" readonly="readonly" :value="$receivingReport ?? null"/>
                        <x-forms.input label="EWT" name="ewt" cols="4" class="autonum text-end"  :value="$receivingReport ?? null"/>
                        <x-forms.input label="A/P" name="ap" cols="4" class="autonum text-end" :value="$receivingReport ?? null"/>
                ...
                <div class="col-md-8">
                    <table class="table table-striped table-sm table-bordered" id="details-table">
                        ...
                        <tbody>
                        @forelse($receivingReport->details as $detail)
    ...
    @include('fg-inventory.receiving-report.t-details')
@endsection
...
@section('scripts')
    <script src="{{asset('js/fg/receiving-reports.js')}}"></script>
    <script type="text/javascript">
        ...
        $("#edit-receiving-report-form").submit(function (e) {
            e.preventDefault();
            let form = $(this);
            let uri = '{{route("receiving-report.update",$receivingReport->uuid)}}';
            loading_btn(form);
            $.ajax({
                url : uri,
                data : form.serialize(),
