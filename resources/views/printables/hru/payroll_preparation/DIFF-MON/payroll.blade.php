...
@section('wrapper')
    ...
    <div style="font-family: Cambria">
        @php
            ...
            $colspan = 5 + $chunkedIncentives->count() + $chunkedDeductions->count() + 2;
            $sumGrand = [];
            $sumGrandSundry = [];
            $sumGrand['pay15'] = null;
            $sumGrand['pay30'] = null;
            ...
            <table style="width: 100%" class=" tbl-padded">
                ...
                <tr>
                    ...
                    @foreach($chunkedDeductions as $grp)
                        <th style="padding: 0; border: none"></th>
                    @endforeach
                    <th style="padding: 0; border: none;width: 15%"></th>
                ...
                <tr>
                    <td colspan="{{$colspan}}">
                        <table style="width: 100%;">
                            <tr>
                                <td style="width: 20%;">
                                    <p class="no-margin text-strong">GENERAL PAYROLL ({{\App\Swep\Helpers\Arrays::payrollTypes()[$payrollMaster->type]}})- {{implode(', ',\Request::get('payrollGroupsSelected'))}}</p>
                ...
                <tr>
                    ...
                    <th class="text-center">PREVIOUS SALARY</th>
                    ...
                    @if(!empty($payrollEmployeesPerRc))
                        ...
                        @forelse($payrollEmployeesPerRc as $payrollEmployee /** @var App\Models\HRU\PayrollMasterEmployees $payrollEmployee **/)
                            <tr>
                                ...
                                <td class="text-right text-top">{{Helper::toNumber($payrollEmployee->diff_old_monthly_basic)}}</td>
                                <td class="text-right text-top">{{Helper::toNumber($payrollEmployee->diff_new_monthly_basic)}}</td>
                                <td class="text-right text-top">{{Helper::toNumber($payrollEmployee->diff_gross)}}</td>
                                <td class="text-right text-top">{{Helper::toNumber($payrollEmployee->diff_days)}}</td>
                                @forelse($headerIncentives as $headerIncentive)
                                ...
                                <td class="text-right text-top text-strong {{$payrollEmployee->pay15 < 0 ? 'text-red' : ''}}">
                        ...
                        <tr>
                            ...
                            <td class="b-top text-top text-right">{{Helper::toNumber($rc15 = $payrollEmployeesPerRc->sum('diff_old_monthly_basic'))}}</td>
                            <td class="b-top text-top text-right">{{Helper::toNumber($rc15 = $payrollEmployeesPerRc->sum('diff_new_monthly_basic'))}}</td>
                            <td class="b-top text-top text-right">{{Helper::toNumber($rc15 = $payrollEmployeesPerRc->sum('diff_gross'))}}</td>
                            <td class="b-top text-top text-right"></td>
                            @forelse($headerIncentives as $headerIncentive)
                                <td class="text-right b-top text-top">
                                    {{
                                        Helper::toNumber(
