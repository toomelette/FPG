                        ...
                        <tr>
                            <td colspan="{{$colspan}}">
                                <table style="width: 100%;">
                                    <tr>
                                        <td style="width: 20%;">
                                            <p class="no-margin text-strong">CONSOLIDATED DIFFERENTIAL - {{$code}} (PS)</p>
            ...
            <div style="break-after: page">
                @foreach($tree as $group => $rcs)
                    <table style="width: 100%; break-after: page" class=" tbl-padded">
                        ...
                        <tr>
                            <td colspan="{{$colspan}}">
                                <table style="width: 100%;">
                                    <tr>
                                        <td style="width: 20%;">
                                            <p class="no-margin text-strong">CONSOLIDATED DIFFERENTIAL - {{$code}} (GS)</p>
                                            <p>PAY PERIOD: {{Carbon::parse($payrollMasters->pluck('date')->sort()->first())->format('M. Y')}} to {{Carbon::parse($payrollMasters->pluck('date')->sort()->last())->format('M. Y')}}</p>
                                        </td>
                                        <td class="text-center">
                                            <p class="no-margin">REPUBLIC OF THE PHILIPPINES</p>
                            ...
                            @if(!empty($payrollEmployeesPerRc))
                                ...
                                @forelse($payrollEmployeesPerRc as $payrollEmployee /** @var App\Models\HRU\PayrollMasterEmployees $payrollEmployee **/)
                                    <tr>
                                        ...
                                        @forelse($payrollMasters as $payrollMaster)
                                            <td class="text-right text-top">
                                                {{ Helper::toNumber($payrollMaster->hmtDetails->where('code','=',$code)->where('employeePayroll.employee_slug','=',$payrollEmployee->employee_slug)->sum('govt_share')) }}
                                            </td>
                                        @empty
                                        @endforelse
                                        ...
                                        <td class="text-right text-top text-strong">
                                            {{
                                                Helper::toNumber($payrollMasters->sum(function ($payrollMaster) use($payrollEmployee,$code){
                                                    return $payrollMaster->hmtDetails->where('code','=',$code)->where('employeePayroll.employee_slug','=',$payrollEmployee->employee_slug)->sum('govt_share');
                                                }))
                                            }}
                                        </td>
                                    </tr>
                                ...
                                <tr>
                                    ...
                                    @forelse($payrollMasters as $payrollMaster)
                                        <td class="text-right b-top">
                                            {{ Helper::toNumber($payrollMaster->hmtDetails->where('code','=',$code)->where('employeePayroll.saved_employee_data.resp_center','=',$rc->first()->resp_center)->sum('govt_share') )  }}
                                        </td>
                                    @empty
                                    @endforelse
                                    <td class="text-right text-top b-top text-strong">
                                        {{
                                            Helper::toNumber($payrollMasters->sum(function ($payrollMaster) use($payrollEmployee,$rc,$code){
                                                return $payrollMaster->hmtDetails->where('code','=',$code)->where('employeePayroll.saved_employee_data.resp_center','=',$rc->first()->resp_center)->sum('govt_share');
                                            }) )
                                        }}
                                    </td>
                                </tr>
                        ...
                        <tr class="text-strong">
                            ...
                            @forelse($payrollMasters as $payrollMaster)
                                <td class="text-right b-top text-strong">
                                    {{ Helper::toNumber($payrollMaster->hmtDetails->where('code','=',$code)->whereIn('employeePayroll.saved_employee_data.resp_center',$rcs->keys()->toArray())->sum('govt_share') )  }}
                                </td>
                            @empty
                            @endforelse
                            ...
                            <td class="text-right text-top b-top text-strong">
                                {{
                                    Helper::toNumber($payrollMasters->sum(function ($payrollMaster) use($payrollEmployee,$rcs,$code){
                                        return $payrollMaster->hmtDetails->where('code','=',$code)->whereIn('employeePayroll.saved_employee_data.resp_center',$rcs->keys()->toArray())->sum('govt_share');
                                    }))
                                }}
                            </td>
                ...
                <table style="width: 100%" class="tbl-padded">
                    ...
                    <tr class="text-strong">
                        ...
                        @forelse($payrollMasters as $payrollMaster)
                            <th class="text-right b-top">
                                {{ Helper::toNumber($payrollMaster->hmtDetails->where('code','=',$code)->sum('govt_share'))  }}
                            </th>
                        ...
                        <td class="text-right text-top b-top">
                            {{
                                Helper::toNumber($payrollMasters->sum(function ($payrollMaster) use($payrollEmployee,$rcs,$code){
                                    return $payrollMaster->hmtDetails->where('code','=',$code)->sum('govt_share');
                                }))
                            }}
                        </td>
