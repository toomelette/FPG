...
@section('wrapper')
    ...
    <div style="font-family: Cambria; text-align: justify; font-size: 16px">
        <div style="break-after: page">
            ...
            <p style="padding-left: 30px; padding-right: 20px">
                <b>{{$hrRequest->employee->full['FMiLE']}}</b>, of legal age, Filipino, {{Str::lower(request('civil_status'))}} and a resident of {{Str::of(request('address'))}}, now and hereinafter referred to as the <b>“SECOND PARTY”</b>.
            </p>
            ...
            <p style="text-indent: 30px;">
                <b>WHEREAS</b>, through {{$hrRequest->document_fields['memo_code'] ?? null}} dated {{Helper::dateFormat($hrRequest->document_fields['memo_date'] ?? null,'F d, Y')}}, Deputy Administrator Atty. Brando D. Noroña approved the request of the <b>{{$hrRequest->document_fields['department'] ?? ''}}</b> for the renewal of the SECOND PARTY for a period of six ({{Carbon::parse($hrRequest->document_fields['contract_start'])->diffInMonths(Carbon::parse($hrRequest->document_fields['contract_end'])) + 1 }}) months from {{Carbon::parse($hrRequest->document_fields['contract_start'])->format('F d, Y')}} July 1, 2025 to {{Carbon::parse($hrRequest->document_fields['contract_end'])->format('F d, Y')}}.
            </p>
            ...
            <ol>
                ...
                <li>
                    That as contract of service personnel, the <b>SECOND PARTY</b> shall be assigned to the <b>{{$hrRequest->document_fields['assignment']}}</b> and is obliged to perform the duties and responsibilities of the position, hereto attached as <b>Annex “A”</b>.
                </li>
                <li>
                    That the <b> SECOND PARTY</b> shall report on site, at the <b>{{$hrRequest->document_fields['assignment']}}</b> from Monday to Friday, and shall work a minimum of eight (8) hours a day.
                </li>
                <li>
        ...
        <div style="break-after: page">
            ...
            <table style="width: 100%; font-size: inherit">
                <tr>
                    ...
                    <td style="width: 50%;" class="text-center">
                        <p class="no-margin"><b><u>{{$hrRequest->employee->full['FMiLE']}}</u></b></p>
                        SECOND PARTY
                    </td>
                </tr>
            </table>
            ...
            <p class="text-center text-strong no-margin">
                <u>{{$hrRequest->document_fields['funds_available']}}</u>
            </p>
            <p class="text-center">
                {{$hrRequest->document_fields['funds_available_position']}}
            </p>
        ...
        <table style="width: 100%; font-size: inherit" class="tbl-padded">
            ...
            <tr>
                <td>
                    <b><u>{{$hrRequest->employee->full['FMiLE']}}</u></b>
                </td>
                <td>
                    <u>{{request('valid_id')}}</u>
                </td>
                <td>
                    <u>{{request('valid_id_issued_at')}}</u>
                </td>
            </tr>
        </table>
        <br>
